...
def by_iteration(data, complete_with='zero', cumsum=False, fixed_max_len=None):
    # 1. Formating data
...
def mean_confidence_interval(data, by_='iteration', confidence=0.99):
    a = 1.0 * np.array(data)
    if by_=='iteration':
        n = len(a)-1
        m, se = np.mean(a,axis=0), scipy.stats.sem(a)
        h = scipy.stats.t.interval(confidence=confidence, df=n,loc=m,scale=se)
        l, u = h[0], h[1]
    elif by_=='experiment':
        n = len(a)-1
    ...
    if target_data == 'cost':
        ...
        for method in results:
            ...
            for exp in results[method]:
                y[method].append(sum(exp['reward'])/exp['it'][-1])
