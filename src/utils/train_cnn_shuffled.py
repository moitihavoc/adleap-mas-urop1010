...
def shuffle_episode_traces(episode):
    """Apply one fixed spatial permutation to trace channels in an episode."""
    spatial_size = episode[0]['obs'].shape[-1] * episode[0]['obs'].shape[-2]
    permutation = torch.randperm(spatial_size)

    shuffled_episode = []
    for transition in episode:
        shuffled_transition = transition.copy()
        observation = transition['obs'].clone()
        trace_channels = observation[3:8].reshape(5, spatial_size)
        observation[3:8] = trace_channels[:, permutation].reshape_as(observation[3:8])
        shuffled_transition['obs'] = observation
        shuffled_episode.append(shuffled_transition)

    return shuffled_episode
...
def train():
    ...
    transitions = []
    for ep in episodes:
        transitions.extend(shuffle_episode_traces(ep))
    ...
    save_path = "src/models/shuffled_trace_cnn_weights.pth"
