...
class IPRPOMCP(object):
    ...
    def simulate(self, node):
        ...
        action = node.select_ucb_action(mode=self.target) 
    ...
    def search(self, node, agent):
        ...
        return node.get_best_ucb_action(self.target)
...
def iprpomcp_planning(env, agent, max_depth=20, max_it=1000, **kwargs):    
    ...
    iprpomcp = IPRPOMCP(max_depth, max_it, kwargs) if 'iprpomcp' not \
     in agent.smart_parameters else agent.smart_parameters['iprpomcp']
    
    # - planning
    next_action, info = iprpomcp.planning(copy_env,agent)

    # 3. Updating the search tree
    agent.smart_parameters['iprpomcp'] = iprpomcp
    agent.smart_parameters['count'] = info
    return next_action,None

def iprpomcp_type_belief(agent):    
    iprpomcp = None if 'iprpomcp' not \
     in agent.smart_parameters else agent.smart_parameters['iprpomcp']

    typebelief = {}
    if iprpomcp is not None:
        pf = iprpomcp.root.particle_filter
        if len(pf) == 0:
            return typebelief
