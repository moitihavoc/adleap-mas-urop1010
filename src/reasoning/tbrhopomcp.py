...
class TBRhoPOMCP(object):
    ...
    def __init__(self,max_depth,max_it,kwargs):
        ...
        self.time_budget = time_budget if time_budget is not None else 3
...
def tbrhopomcp_planning(env, agent, max_depth=20, max_it=250, **kwargs):    
    # 1. Setting the environment for simulation
    copy_env = env.copy()
    copy_env.simulation = True

    # 2. Planning
    rhopomcp = TBRhoPOMCP(max_depth, max_it, kwargs) if 'rhopomcp' not \
