...
from src.reasoning.node import find_new_information_root, particle_revigoration
...
def iucbpomcp_planning(env, agent, max_depth=20, max_it=1000, **kwargs):    
...
def iucbpomcp_type_belief(agent):    
    iucbpomcp = None if 'iucbpomcp' not \
     in agent.smart_parameters else agent.smart_parameters['iucbpomcp']

    typebelief = {}
    if iucbpomcp is not None:
        pf = iucbpomcp.root.particle_filter
        if len(pf) == 0:
            return typebelief
