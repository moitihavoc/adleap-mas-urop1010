...
class LIBPOMCP(object):
    ...
    def simulate(self, node):
        ...
        future_reward, _ = self.simulate(observation_node)
        R = reward + (self.discount_factor * future_reward)

        # - node update
        node.particle_filter.append(node.state)
        
        # -  adding just the local observation found
        node.add_to_observation_distribution([observation])
...
def libpomcp_planning(env, agent, max_depth=20, max_it=1000, **kwargs):    
    ...
    libpomcp = LIBPOMCP(max_depth, max_it, kwargs) if 'libpomcp' not \
     in agent.smart_parameters else agent.smart_parameters['libpomcp']
    
    # - planning
    next_action, info = libpomcp.planning(copy_env,agent)

    # 3. Updating the search tree
    agent.smart_parameters['libpomcp'] = libpomcp
    agent.smart_parameters['count'] = info
    return next_action,None

def libpomcp_type_belief(agent):    
    libpomcp = None if 'libpomcp' not \
     in agent.smart_parameters else agent.smart_parameters['libpomcp']

    typebelief = {}
    if libpomcp is not None:
        pf = libpomcp.root.particle_filter
        if len(pf) == 0:
            return typebelief

        for particle in pf:
            for ag in particle.components['agents']:
                if ag.index != agent.index:
                    if ag.index not in typebelief:
                        typebelief[ag.index] = {}

                    if ag.type in typebelief[ag.index]:
